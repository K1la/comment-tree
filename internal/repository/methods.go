...
func (r *Repository) CreateComment(ctx context.Context, comment *model.Comment) (*model.Comment, error) {
	...
	zlog.Logger.Debug().Interface("parent_id", comment.ParentID).Msg("insert comment into db")
...
func (r *Repository) GetComments(ctx context.Context, parentID *uuid.UUID, search, sort string, limit, offset int) ([]model.Comment, error) {
	query := `
		SELECT id, parent_id, content, created_at, updated_at
		FROM comments 
		WHERE 1=1
